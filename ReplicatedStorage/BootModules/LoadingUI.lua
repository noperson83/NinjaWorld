...
function LoadingUI.start(config)
	config = config or {}
	...
	if parentGui then
		...
		warn("LoadingUI: PlayerGui not available, deferring intro UI parent")
		task.spawn(function()
			local target = getPlayerGui(player)
			if target and gui.Parent ~= target then
				gui.Parent = target
			elseif not target then
				warn("LoadingUI: Failed to find PlayerGui; destroying intro UI")
				gui:Destroy()
			end
		end)
	end
