	...
	-- UI Configuration with improved responsive settings
	local UI_CONFIG = {
		-- Colors
		COMBAT_COLOR = Color3.fromRGB(200, 40, 40),
		ABILITY_COLOR = Color3.fromRGB(50, 150, 220),
		MOVEMENT_COLOR = Color3.fromRGB(140, 160, 0),
		JUMP_COLOR = Color3.fromRGB(60, 210, 120),
		BACKGROUND_COLOR = Color3.fromRGB(30, 30, 30),
		TEXT_COLOR = Color3.fromRGB(255, 255, 255),
		KEYBIND_COLOR = Color3.fromRGB(200, 200, 200),
		DISABLED_COLOR = Color3.fromRGB(100, 100, 100),
		...
		-- Layout - Base sizes (will be scaled)
		...
		-- NEW: Tablet-specific sizes
		TABLET_BUTTON_SIZE = UDim2.new(0, 100, 0, 80),
		TABLET_JUMP_SIZE = UDim2.new(0, 120, 0, 100),
		...
		-- Responsive scaling - IMPROVED
		SCALE_REFERENCE_WIDTH = 1920,  -- Reference width for desktop
		SCALE_REFERENCE_HEIGHT = 1080, -- Reference height for desktop
		SCALE_MIN = 0.6,  -- Allow smaller on phones
		SCALE_MAX = 1.3,

		-- NEW: Platform-specific scale multipliers
		PHONE_SCALE_MULTIPLIER = 0.85,
		TABLET_SCALE_MULTIPLIER = 0.95,
		DESKTOP_SCALE_MULTIPLIER = 1.0,
		...
		ENABLE_HAPTICS = true,
		ENABLE_SOUNDS = true,
		BUTTON_SOUND_ID = "rbxassetid://9112860835",
		JUMP_SOUND_ID = "rbxassetid://9112860835",
	...
	local FAN_CONFIG = {
		...
		TABLET_OUTER_RADIUS_OFFSET = 55,

		-- Margins for different platforms
		JUMP_MARGIN = 90,
		MOBILE_JUMP_MARGIN = 60,
		TABLET_JUMP_MARGIN = 75,
		DESKTOP_JUMP_MARGIN = 95,

		TOGGLE_MARGIN = 36,
		MOBILE_TOGGLE_MARGIN = 28,
		TABLET_TOGGLE_MARGIN = 32,
		DESKTOP_TOGGLE_MARGIN = 40,

		MIN_HORIZONTAL_MARGIN = 18,
		MOBILE_MIN_HORIZONTAL_MARGIN = 12,
		TABLET_MIN_HORIZONTAL_MARGIN = 15,

		OUTER_MIN_HORIZONTAL_MARGIN = 28,
		MOBILE_OUTER_MIN_HORIZONTAL_MARGIN = 20,
		TABLET_OUTER_MIN_HORIZONTAL_MARGIN = 24,

		VERTICAL_SHIFT = 12,
		MOBILE_VERTICAL_SHIFT = 8,
		TABLET_VERTICAL_SHIFT = 10,

		OUTER_VERTICAL_SHIFT = 44,
		MOBILE_OUTER_VERTICAL_SHIFT = 32,
		TABLET_OUTER_VERTICAL_SHIFT = 38,
		...
		TABLET_TOGGLE_SIZE = UDim2.new(0, 48, 0, 48),
	}
	...
	local orientationConnection = nil
	...
	local currentDeviceType = "desktop"  -- NEW: Track device type
	local currentOrientation = "landscape"  -- NEW: Track orientation
	...
	-- NEW: Detect device type more accurately
	local function detectDeviceType()
		...
		local maxAxis = math.max(width, height)
		local aspectRatio = maxAxis / minAxis

		-- Determine orientation
		local orientation = width > height and "landscape" or "portrait"
		...
		if hasTouch then
			-- Phone: smaller screen, typically > 16:9 aspect ratio
			if minAxis < 600 or aspectRatio > 2.0 then
				return "phone", orientation
			end
			-- Tablet: larger touch screen
			if minAxis >= 600 and minAxis < 1200 then
				return "tablet", orientation
			end
			-- Large tablet or touch desktop
			return "tablet", orientation
		end
	...
	-- NEW: Get platform-specific scale multiplier
	local function getPlatformScaleMultiplier()
		if currentDeviceType == "phone" then
			return UI_CONFIG.PHONE_SCALE_MULTIPLIER
		elseif currentDeviceType == "tablet" then
			return UI_CONFIG.TABLET_SCALE_MULTIPLIER
		else
			return UI_CONFIG.DESKTOP_SCALE_MULTIPLIER
		end
	end
	...
	local function updateUIScale()
		...
		currentDeviceType, currentOrientation = detectDeviceType()

		local viewportSize = camera.ViewportSize
		local minAxis = math.min(viewportSize.X, viewportSize.Y)
		local maxAxis = math.max(viewportSize.X, viewportSize.Y)

		-- Use different scaling logic based on device
		local scale
		if currentDeviceType == "phone" then
			-- Scale based on the smaller axis for phones
			scale = minAxis / 600
		elseif currentDeviceType == "tablet" then
			...
			-- Desktop scaling
			scale = minAxis / UI_CONFIG.SCALE_REFERENCE_HEIGHT
		end

		-- Apply platform multiplier
		scale = scale * getPlatformScaleMultiplier()

		-- Clamp
		currentUIScale = math.clamp(scale, UI_CONFIG.SCALE_MIN, UI_CONFIG.SCALE_MAX)
		return currentUIScale
	end
	...
	-- NEW: Get config value based on device type
	local function getConfigValue(mobileName, tabletName, desktopName, baseName)
		if currentDeviceType == "phone" and mobileName then
			return mobileName
		elseif currentDeviceType == "tablet" and tabletName then
			return tabletName
		elseif currentDeviceType == "desktop" and desktopName then
			return desktopName
		end
		return baseName
	end

	local function getJumpMargin()
		local baseMargin = getConfigValue(
			FAN_CONFIG.MOBILE_JUMP_MARGIN,
			FAN_CONFIG.TABLET_JUMP_MARGIN,
			FAN_CONFIG.DESKTOP_JUMP_MARGIN,
			FAN_CONFIG.JUMP_MARGIN
		)
		return scaleNumber(baseMargin or 0)
	end

	local function getToggleMargin()
		local baseMargin = getConfigValue(
			FAN_CONFIG.MOBILE_TOGGLE_MARGIN,
			FAN_CONFIG.TABLET_TOGGLE_MARGIN,
			FAN_CONFIG.DESKTOP_TOGGLE_MARGIN,
			FAN_CONFIG.TOGGLE_MARGIN
		)
		return scaleNumber(baseMargin or 0)
	end

	local function getToggleSize()
		local baseSize = getConfigValue(
			FAN_CONFIG.MOBILE_TOGGLE_SIZE,
			FAN_CONFIG.TABLET_TOGGLE_SIZE,
			FAN_CONFIG.TOGGLE_SIZE,
			FAN_CONFIG.TOGGLE_SIZE
		)
		return scaleUDim2(baseSize)
	end

	local function getFanRadius()
		local baseRadius = getConfigValue(
			FAN_CONFIG.MOBILE_RADIUS,
			FAN_CONFIG.TABLET_RADIUS,
			FAN_CONFIG.DESKTOP_RADIUS,
			FAN_CONFIG.MOBILE_RADIUS
		)
		return scaleNumber(baseRadius or 0)
	end

	local function getOuterFanRadius(baseRadius)
		local offset = getConfigValue(
			FAN_CONFIG.MOBILE_OUTER_RADIUS_OFFSET,
			FAN_CONFIG.TABLET_OUTER_RADIUS_OFFSET,
			FAN_CONFIG.OUTER_RADIUS_OFFSET,
			FAN_CONFIG.OUTER_RADIUS_OFFSET
		)
		return baseRadius + scaleNumber(offset or 0)
	end

	local function getHorizontalMargin(isOuter)
		local baseMargin
		if isOuter then
			baseMargin = getConfigValue(
				FAN_CONFIG.MOBILE_OUTER_MIN_HORIZONTAL_MARGIN,
				FAN_CONFIG.TABLET_OUTER_MIN_HORIZONTAL_MARGIN,
				FAN_CONFIG.OUTER_MIN_HORIZONTAL_MARGIN,
				FAN_CONFIG.OUTER_MIN_HORIZONTAL_MARGIN
			)
		else
			baseMargin = getConfigValue(
				FAN_CONFIG.MOBILE_MIN_HORIZONTAL_MARGIN,
				FAN_CONFIG.TABLET_MIN_HORIZONTAL_MARGIN,
				FAN_CONFIG.MIN_HORIZONTAL_MARGIN,
				FAN_CONFIG.MIN_HORIZONTAL_MARGIN
			)
		end
		return scaleNumber(baseMargin or 0)
	...
	local function getVerticalShift(isOuter)
		local baseShift
		if isOuter then
			baseShift = getConfigValue(
				FAN_CONFIG.MOBILE_OUTER_VERTICAL_SHIFT,
				FAN_CONFIG.TABLET_OUTER_VERTICAL_SHIFT,
				FAN_CONFIG.OUTER_VERTICAL_SHIFT,
				FAN_CONFIG.OUTER_VERTICAL_SHIFT
			)
		else
			baseShift = getConfigValue(
				FAN_CONFIG.MOBILE_VERTICAL_SHIFT,
				FAN_CONFIG.TABLET_VERTICAL_SHIFT,
				FAN_CONFIG.VERTICAL_SHIFT,
				FAN_CONFIG.VERTICAL_SHIFT
			)
		end
	...
	local function shouldUseMobileLayout()
		return currentDeviceType ~= "desktop" or forceActionsVisible
	end

	local function shouldUseDesktopLayout()
		return currentDeviceType == "desktop" and not forceActionsVisible
	end
	...
	local function createFanToggleButton()
		...
		toggle.Size = getToggleSize() or scaleUDim2(FAN_CONFIG.TOGGLE_SIZE)
	...
	local function createStylizedButton(buttonDef)
		...
		-- Get appropriate size for current device type
		local baseSize
		if buttonDef.category == "jump" then
			if currentDeviceType == "desktop" and UI_CONFIG.DESKTOP_JUMP_SIZE then
				baseSize = UI_CONFIG.DESKTOP_JUMP_SIZE
			elseif currentDeviceType == "tablet" and UI_CONFIG.TABLET_JUMP_SIZE then
				baseSize = UI_CONFIG.TABLET_JUMP_SIZE
			elseif currentDeviceType == "phone" and UI_CONFIG.MOBILE_JUMP_SIZE then
				baseSize = UI_CONFIG.MOBILE_JUMP_SIZE
			else
				baseSize = UI_CONFIG.JUMP_BUTTON_SIZE
			end
		else
			if currentDeviceType == "desktop" and UI_CONFIG.DESKTOP_BUTTON_SIZE then
				baseSize = UI_CONFIG.DESKTOP_BUTTON_SIZE
			elseif currentDeviceType == "tablet" and UI_CONFIG.TABLET_BUTTON_SIZE then
				baseSize = UI_CONFIG.TABLET_BUTTON_SIZE
			elseif currentDeviceType == "phone" and UI_CONFIG.MOBILE_BUTTON_SIZE then
				baseSize = UI_CONFIG.MOBILE_BUTTON_SIZE
			else
				baseSize = UI_CONFIG.BUTTON_SIZE
			end
		end
		...
		if currentDeviceType == "desktop" and UI_CONFIG.DESKTOP_PADDING then
		...
		-- Keybind label (hide on touch devices)
		...
		keybindLabel.Visible = currentDeviceType == "desktop"
	...
	local function setupButtonAnimations(button)
		...
		registerConnection(buttonConnections, button.MouseEnter:Connect(function()
			if currentDeviceType == "desktop" then
		...
		local function handleRelease()
			tweenScale(1, 0.12, UI_CONFIG.BOUNCE_EASING)
		end
	...
	playButtonSound = function(soundId)
	...
	applySpeedState = function(forceRemote)
	...
	ensureCharacterTracking = function()
		local player = Players.LocalPlayer
		if not player then
			return
		end
	...
	updateFanLayout = function(animated)
		if not currentActionsFrame then
			return
		end
		...
		local radius = getFanRadius()
		local jumpSize = jumpButtonRef.Size
		local margin = getJumpMargin()
		local baseOffsetX = -(jumpSize.X.Offset / 2) - margin
		local baseOffsetY = -(jumpSize.Y.Offset / 2) - margin

		-- Adjust positioning based on orientation for portrait mode
		local edgePadding = scaleNumber(currentOrientation == "portrait" and 20 or 25)
		...
		if #abilityButtons > 0 then
			abilityRadius = getOuterFanRadius(radius)
		end

		local innerMargin = getHorizontalMargin(false)
		local abilityMargin = getHorizontalMargin(true)
		...
		local innerShiftY = getVerticalShift(false)
		local abilityShiftY = getVerticalShift(true)
		...
		if toggleButtonRef then
			local toggleSize = getToggleSize()
			if toggleSize then
				toggleButtonRef.Size = toggleSize
			else
				toggleSize = toggleButtonRef.Size
			end
			local toggleMargin = getToggleMargin()
	...
	setFanOpen = function(state, animated)
	...
	local function disableDefaultJump()
		...
		if currentDeviceType ~= "desktop" or forceActionsVisible then
	...
	updateCooldownVisuals = function()
		for buttonName, cooldown in pairs(cooldowns) do
			...
			if button then
				...
				if remaining > 0 then
					...
					button.BackgroundColor3 = button:GetAttribute("OriginalColor") or UI_CONFIG.COMBAT_COLOR
	...
	local function ensureActions()
		...
		if shouldUseDesktopLayout() then
			...
			local actionInset = scaleNumber(currentOrientation == "portrait" and 15 or 20)
		...
		local allowJumpButton = currentDeviceType ~= "desktop"

		if not allowJumpButton and UI_CONFIG.SHOW_ON_DESKTOP and UI_CONFIG.SHOW_DESKTOP_JUMP and shouldUseDesktopLayout() then
		...
		if jumpButtonRef and not shouldUseDesktopLayout() then
			local margin = getJumpMargin()
			local jumpSize = jumpButtonRef.Size
		...
		if #fanButtons > 0 and not shouldUseDesktopLayout() then
	...
	function ActionUI.init()
		...
		if actions then
			-- Combat actions
			actionMap = {
				PunchButton = {func = CombatController.perform, param = "Punch", cooldown = 1},
				...
				CrouchButton = {func = CombatController.perform, param = "Crouch", cooldown = 0},
		...
		-- Auto-resize on viewport changes
		local camera = workspace.CurrentCamera
		if camera then
		...
		-- Monitor orientation changes (important for tablets/phones)
		if orientationConnection then
			orientationConnection:Disconnect()
		end

		if camera then
			orientationConnection = camera:GetPropertyChangedSignal("ViewportSize"):Connect(function()
				local oldOrientation = currentOrientation
				local oldDeviceType = currentDeviceType
				updateUIScale()  -- This updates orientation and device type

				-- Only reinit if device type or orientation actually changed
				if oldOrientation ~= currentOrientation or oldDeviceType ~= currentDeviceType then
					print("Device changed:", oldDeviceType, "->", currentDeviceType, "| Orientation:", oldOrientation, "->", currentOrientation)
					task.delay(0.15, ActionUI.init)
				end
			end)
		end
	...
	-- NEW: Get current device info (useful for debugging or external scripts)
	function ActionUI.getDeviceInfo()
		return {
	...
	-- NEW: Force a specific device layout (for testing)
	function ActionUI.forceDeviceType(deviceType)
		if deviceType == "phone" or deviceType == "tablet" or deviceType == "desktop" then
